#!/usr/bin/env bash
# Launch the libero_spatial_3bowl eval (extra-distractor variant) across multiple
...
#
# Usage:
#   bash eval_libero_spatial_3bowl_multigpu.sh                    # GPUs 0..7
#   GPUS="0,1,2,3,4" bash eval_libero_spatial_3bowl_multigpu.sh   # specific GPUs
#   USE_EXPLICIT_PROMPT=False GPUS="0,1,2,3,4" bash eval_libero_spatial_3bowl_multigpu.sh
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKSPACE_ROOT="$(cd "${SCRIPT_DIR}/../.." && pwd)"
SINGLE_SCRIPT="${SCRIPT_DIR}/eval_libero_spatial_3bowl.sh"
...
echo "Console logs: ${LOG_DIR}/multigpu-3bowl-${STAMP}-shard*.out"

pids=()
for idx in "${!GPU_ARR[@]}"; do
  gpu="${GPU_ARR[$idx]}"
  out="${LOG_DIR}/multigpu-3bowl-${STAMP}-shard${idx}of${NUM_SHARDS}-gpu${gpu}.out"
...
for i in "${!pids[@]}"; do
  if ! wait "${pids[$i]}"; then
    echo "Shard ${i} (GPU ${GPU_ARR[$i]}) FAILED — see ${LOG_DIR}/multigpu-3bowl-${STAMP}-shard${i}of${NUM_SHARDS}-gpu${GPU_ARR[$i]}.out" >&2
    fail=1
  fi
done
