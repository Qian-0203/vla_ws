#!/usr/bin/env bash
# Evaluate the libero_spatial LoRA baseline on the libero_spatial_3bowl_open variant:
...
TASK_SUITE_NAME="${TASK_SUITE_NAME:-libero_spatial_3bowl_open}"
...
if [[ "${USE_EXPLICIT_PROMPT}" == "True" ]]; then
  RUN_ID_NOTE="${RUN_ID_NOTE:-explicit_instructions_3bowl_open}"
else
  RUN_ID_NOTE="${RUN_ID_NOTE:-default_instructions_3bowl_open}"
fi
